...
- (void)showGiftShowViewWithModel:(LDSGiftModel *)giftModel completeBlock:(completeShowViewBlock)completeBlock {
    ...
    [self.giftImageView sd_setImageWithURL:[NSURL URLWithString:giftModel.userIcon] placeholderImage:[UIImage imageNamed:@""]];
    self.whoNameLabel.text = giftModel.userName;
    [self.whoNameLabel sizeToFit];
    self.sendWhatLabel.text = [NSString stringWithFormat:@"送 %@",giftModel.giftName];
